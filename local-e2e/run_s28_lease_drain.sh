...
if [[ -z "${non_committer_writer_services}" ]]; then
  ...
  printf 'writer_services=%s append_services=%s enabled_services=%s lease_conflicts=%s\n' \
    "${writer_services}" "${append_services}" "${enabled_services}" "${lease_conflicts}" >&2
  exit 1
fi

if [[ -z "${append_services}" ]]; then
  printf 'expected a committer to append staged rows\n' >&2
  printf 'writer_services=%s non_committer_writer_services=%s enabled_services=%s lease_conflicts=%s\n' \
    "${writer_services}" "${non_committer_writer_services}" "${enabled_services}" "${lease_conflicts}" >&2
  exit 1
fi

if [[ -n "${lease_conflicts}" ]]; then
  printf 'unexpected etcd committer lease conflict for same changefeed\n' >&2
  printf 'writer_services=%s non_committer_writer_services=%s append_services=%s enabled_services=%s lease_conflicts=%s\n' \
    "${writer_services}" "${non_committer_writer_services}" "${append_services}" "${enabled_services}" "${lease_conflicts}" >&2
  exit 1
fi
...
printf 'S28_LEASE_DRAIN_PASS cf=%s summary=%s staged_after=%s staged_after_remove=%s writer_services=%s non_committer_writer_services=%s append_services=%s enabled_services=%s\n' \
  "${CF}" \
  "${summary}" \
  "${staged_after}" \
  "${staged_after_remove}" \
